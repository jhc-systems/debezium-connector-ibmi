...
public class CommitLogProcessor {
    ...
    public static void main(String[] args) throws Exception {
        ...
        if (offset != null && receiver != null) {
            nextPosition = new JournalProcessedPosition(offset, receiver, journal.journalLibrary(),
    ...
    private static JournalProcessedPosition retrieveJorunal(Connect<AS400, IOException> connector, JournalInfo journal,
                                                            RetrieveJournal r, JournalProcessedPosition position)
            throws Exception {
        ...
        if (success) {
            ...
            while (r.nextEntry()) {
                ...
                // log.debug("time {} receiver {} offset {} lib: {} file: {} member: {}",
                // eheader.getTime(), eheader.getReceiver(), eheader.getEndOffset(), lib, file,
                // member);
                ...
                switch (entryType) {
                    ...
                    case BEFORE_IMAGE, ROLLBACK_BEFORE_IMAGE:
                        // log.debug("update row old values lib: {} file: {} member: {}", lib, file,
                        // member);
                        // dumpTable(eheader, r, file, lib, member);
                        break;
                    case AFTER_IMAGE, ROLLBACK_AFTER_IMAGE:
                        // log.debug("update row new values lib: {} file: {} member: {}", lib, file,
                        // member);
                        dumpTable(eheader, r, file, lib, member);
                        break;
                    default:
            ...
            // if (eh != null) {
            // log.info("last offset was {}.{}.{}", eh.getSequenceNumber(),
            // eh.getReceiver(), eh.getReceiverLibrary());
            // }
            //
            ...
            if (lastOffset.getReceiver() != null && !journalNow.equals(lastOffset.getReceiver())) {
                log.warn("journal receiver doesn't match at position {} we have journal {} and latest is {} ", position,
                        lastOffset.getReceiver(), journalNow);
            }
