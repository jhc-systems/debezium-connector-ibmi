...
public class As400JdbcConnection extends JdbcConnection implements Connect<Connection, SQLException> {
	...
	private static Field[] JdbcFields = new Field[] {
			...
			As400ConnectorConfig.FORCE_CCSID, As400ConnectorConfig.DATE_FORMAT, As400ConnectorConfig.DB_ERRORS,
			As400ConnectorConfig.THREAD_USED, As400ConnectorConfig.KEEP_ALIVE,

			// don't pop up a GUI prompt
			Field.create("prompt", "prompt", "do you want a GUI prompt for the password if the password is wrong",
					false), };
	...
	public List<FileFilter> shortIncludes(String schema, String includes) {
		...
		for (String tableName : incs) {
			...
			// This handles the case where the table name has been specified as
			...
			getSystemName(tableSchema, tableName).ifPresent(x -> r.add(new FileFilter(tableSchema, x)));
		}
		return r;
	}
	...
	protected List<String> readAs400PrimaryKeys(TableId id) throws SQLException {
		final List<String> columns = prepareQueryAndMap(GET_INDEXES, call -> {
	...
	protected Map<TableId, List<Column>> getColumnsDetails(String databaseCatalog, String schemaNamePattern,
			...
			final Set<TableId> viewIds) throws SQLException {
	...
	public Optional<String> getSystemName(String schemaName, String longTableName) {
		...
		if (longToSystemTableName.containsKey(longKey)) {
			...
			try {
				String systemName = prepareQueryAndMap(GET_SYSTEM_TABLE_NAME, call -> {
					call.setString(1, schemaName);
	...
	public String getLongName(String schemaName, String systemName) {
		...
		if (systemToLongTableName.containsKey(systemKey)) {
			...
			try {
				longTableName = prepareQueryAndMap(GET_TABLE_NAME, call -> {
					call.setString(1, schemaName);
					call.setString(2, systemName);
				}, singleResultMapper(rs -> rs.getString(1).trim(),
						String.format("Could not retrieve long table name %s in %s", systemName, schemaName)));
	...
	@Override
	protected Optional<ColumnEditor> readTableColumn(ResultSet columnMetadata, TableId tableId,
			ColumnNameFilter columnFilter) throws SQLException {
		...
		if (columnFilter == null
			...
			// Allow implementation to make column changes if required before being added to
			// table
			column = overrideColumn(column);

			// don't fetch defaults or we will send the default value when not set even
			// though the database value is not set
