...
public class As400DefaultValueConverter implements DefaultValueConverter {
	...
	public Object convert(Column column, String value) {
		...
		case Types.TIMESTAMP_WITH_TIMEZONE:
			throw new UnsupportedOperationException(
					"not yet implemented to default to timestamp and timezone, value was: " + value);
		case Types.TIME:
	...
	private Object convertToDecimal(Column column, String value) {
		return column.scale().map(x -> new BigDecimal(value).setScale(x, RoundingMode.HALF_UP))
				.orElse(new BigDecimal(value));
	}
	...
	private Object convertToBits(String value) {
		final int nums = value.length() / Byte.SIZE + (value.length() % Byte.SIZE == 0 ? 0 : 1);
		final byte[] bytes = new byte[nums];
		for (int i = 0; i < nums; i++) {
			final int s = value.length() - Byte.SIZE < 0 ? 0 : value.length() - Byte.SIZE;
